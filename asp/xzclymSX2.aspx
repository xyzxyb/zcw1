        ...
        DataConn objConn=new DataConn();
		    protected void Page_Load(object sender, EventArgs e)
            {
               ...
               //先根据传过来的flsx_id 查出分类编码			
                DataTable dt_flsxid= objConn.GetDataTable("select 分类编码 from 材料分类属性值表 where flsx_id='"+flsx_id+"'");    //分类 属性
			  
               string clflsx_id = Convert.ToString(dt_flsxid.Rows[0]["分类编码"]);	  //获取分类编码		
			
			   //以分类编码   和分类名称传过来的分类id 查询属性值 
			   DataTable dt_flsx_id = objConn.GetDataTable("select 编号,属性值,flsxz_id from 材料分类属性值表 where 是否启用=1 and 分类编码='"+clflsx_id+"' and flsx_id='"+flsx_id+"' order by 属性编码,编号");    //分类 属性
