    ...
    <script runat="server">
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
		    if (!Page.IsPostBack)
            {
			   ...
			   dt_gysxx = objConn.GetDataTable("select 供应商,单位类型,联系人,联系人手机,联系地址 from 材料供应商信息表 where  gys_id='"+gys_id+"'");
               ...
               objConn.ExecuteSQL(str_updatecounter,false);
                ...
                {
                    //获得代理品牌信息
			      dt_ppxx =objConn.GetDataTable("select 品牌名称,pp_id from 分销商和品牌对应关系表 where fxs_id='"+gys_id+"'");
                  
                    //获得正在分销的材料列表
			      dt_clxx = objConn.GetDataTable("select 显示名,cl_id from 材料表 where pp_id in(select pp_id from  分销商和品牌对应关系表 where fxs_id ='"+gys_id+"') ");
                  
                }
                else  //生厂商则显示旗下品牌和它的分销商
                {
                     //获取品牌信息
			       dt_ppxx = objConn.GetDataTable("select 品牌名称,pp_id from 品牌字典 where 是否启用 = '1' and scs_id='"+gys_id+"'");
                   
                    //获取分销商信息
			        //子查询嵌套 先根据传过来的gys_id查品牌名称  再从品牌字典里查复合条件的gys_id 最后根据复合条件的gys_id查分销商信息
			        dt_fxsxx =objConn.GetDataTable("select 供应商,联系人,联系人手机,联系地址,gys_id from 材料供应商信息表 where gys_id in(select fxs_id from 分销商和品牌对应关系表 where pp_id in(select pp_id from 品牌字典 where scs_id='"+gys_id+"') )");
